...
var fs = require("fs");
var chatDB = require("./persistent_server.js");
    ...
    switch (request.url) {
      case '/classes/messages':
        ...
        if (request.method === "GET"){
          chatDB.query('select username, text, roomname from messages join users on id_users join rooms on id_rooms;',
            function(results) {
          ...
          request.setEncoding();
          request.on('data', chatDB.query('replace into users (username values );',function(results) {''
              messages = {'results': results}
              console.log("Messages: ", messages);
              response.end(JSON.stringify(messages));
          });
            //if it's a new room create new room
            // 'replace into users (username) values (somename);'
            //if it's  a  new user create new user
            //'replace into rooms (roomname) values (someroom);'
            //create new message with user_id and room_id
            //insert into messages (roomname, username, text) values (data.room, data.username, data.text)
